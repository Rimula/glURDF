...
// camera
learnOpenGL::Camera camera(glm::vec3(1.5f, 25.0f, 18.0f));
...
vector<shared_ptr<Model_>> second_robot_meshes; // Added for the second robot
...
int main(int argc, char** argv)
{
    ...
    if (!robot){
        ...
        // 文件路径定义
    const string urdf_file_path_robot2 = "/home/hewei/GLURDF/glURDF/resources/T12/urdf/T12.URDF";

    // 读取和解析第二个URDF文件
    string xml_string_robot2;
    fstream xml_file_robot2(urdf_file_path_robot2, fstream::in);
    while (xml_file_robot2.good())
    {
        string line;
        getline(xml_file_robot2, line);
        xml_string_robot2 += (line + "\n");
    }
    xml_file_robot2.close();

    ModelInterfaceSharedPtr robot2 = parseURDF(xml_string_robot2);
    if (!robot2)
    {
        cerr << "ERROR: Model Parsing the xml for the second robot failed" << endl;
        return -1;
    }
...
////xinzneg
    ...
    unsigned char *data = stbi_load("/home/hewei/GLURDF/glURDF/resources/scenes/sea.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
...
////xinzeng
    ...
    shared_ptr<Model_> second_root_node(new Model_ (link->name + "_2", file_name, link->visual->material->name, 0, init, init, scale, axis, nullptr));
    second_robot_meshes.push_back(second_root_node);
    addChildLinks(link, second_root_node);

    for (int i = 0; i < second_robot_meshes.size(); i++)
    {    
        if (i == 0)
        {
            glm::mat4 tmp = glm::translate(init, glm::vec3(5.0f, 0.0f, 0.0f));  // Place the second robot at a different position
            tmp = glm::rotate(tmp, glm::radians(90.0f), glm::vec3(-1.0f,0.0f,0.0f));  
            tmp = glm::rotate(tmp, glm::radians(90.0f), glm::vec3(0.0f,0.0f,-1.0f));  
            second_robot_meshes[i]->joint_transmat = tmp;
        }        
    }   
    ...
    // Model objModel = loadModel("path_to_your_obj_file.glb");
    // objModels.push_back(make_shared<Model>(objModel));
...
// bool LoadUSDModel(const std::string& filename, GLuint& VAO) {
...
//     for (const auto& prim : stage.root_prims()) { // 使用 GetPrims() 获取所有原始数据
//        // 假设 Mesh 类型存在于 Value 中
//         if (prim.type_name() == "Mesh") { // 根据类型判断
//             const auto& data = prim.data(); // 获取存储的值
//             const auto* mesh = data.as<Mesh>(); // 尝试转换为 Mesh 类型
//             if (mesh) {
//                 // 假设 Mesh 有 GetVertices() 和 GetIndices() 方法
...
void renderScene(const Shader &shader, bool move)
{
    ...
    {          
        glm::mat4 transmat = init;
        transmat = glm::scale(transmat, second_robot_meshes[i]->scale);
        transmat = second_robot_meshes[i]->vis_transmat * transmat;

        for (int j = 0; j < 3; j++)
        {
            if (second_robot_meshes[i]->scale[j] < 0)
                glDisable(GL_CULL_FACE);
        }

        if (!second_robot_meshes[i]->textures_loaded.empty())
            shader.setInt("textures", 1);
        else
            shader.setInt("textures", 0);
        
        transmat = calTransMat(second_robot_meshes[i]) * transmat;
        shader.setVec3("color", second_robot_meshes[i]->color);
        shader.setMat4("model", transmat);
        second_robot_meshes[i]->Draw(shader);
    }
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_RELEASE)
    {
        ...
        camera.Position = glm::vec3(20.0f, 20.0f, 20.0f); // 重新设置摄像机位置
    }
}
